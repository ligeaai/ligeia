...
df2 = df\
    ...
    .drop("time")\
    .select("id","org_unit4","asset","date","timee","temperature","pressure","vibration_x","vibration_y","vibration_motor")

query = df2.selectExpr("CAST(id AS STRING) AS key","to_json(struct(*)) AS value") \
    .writeStream \
    .format("kafka") \
    .option("kafka.bootstrap.servers", "localhost:9092") \
