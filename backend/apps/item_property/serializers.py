        ...
        try:
            for keys in tempt_data[index].keys():
                ...
                if condition_value == "" or condition_value == None:
                    continue
                typeValue = type_of_value.get(
                    tempt_data[index].get(keys).get("VALUE_TYPE")
                )
        ...
        for index in range(0, len(tempt_data)):
            ...
            try:
                ...
                for keys in tempt_keys:
                    ...
                    tempt_data[index].get(keys)["ITEM_TYPE"] = item_data.get(
                        "ITEM_TYPE"
                    )
                    ...
                    tempt_data[index].get(keys)["LAST_UPDT_DATE"] = str(
                        datetime.now()
                    ).split(" ")[0]
                    ...
                    tempt_data[index].get(keys)["PROPERTY_INFO"] = (
                        tempt_data[index].get(keys).get("VALUE_TYPE")
                    )
                    tempt_data[index].get(keys)[typeValue] = (
                        tempt_data[index].get(keys).get("VALUE")
                    )
                    tempt_data[index].get(keys).pop("VALUE_TYPE")
                    tempt_data[index].get(keys).pop("VALUE")
                    item_propertys = item_property.objects.create(
                        **tempt_data[index].get(keys)
                    )
