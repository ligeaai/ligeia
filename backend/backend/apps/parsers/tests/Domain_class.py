        ...
        if (key_type == 'Many'):
            for i in range(len(main_to_text_dict[key])):
                if (main_to_text_dict[key][i]['ToType'].lower() == 'network'):
                    ...
                    main_text = "\n    {}_id = models.ManyToManyField({}, through={})".format(
                        main_to_text_dict[key][i]['ToType'].lower(), main_to_text_dict[key][i]['ToType'].capitalize(),
                        main_to_text_dict[key][i]['Type'].capitalize())
                    main_list.append(main_text)
                    main_text = ""
                    main_from_text1 = "\nfrom pgdbmodel.models.{} import {}".format(main_to_text_dict[key][i]['ToType'].capitalize(),
            ...
            for i in range(len(main_to_text_dict[key])):
                if (main_to_text_dict[key][i].lower() == 'network'):
                    ...
                    main_list.append(main_text)
                    main_text = ""
                    main_from_textt = "\nfrom pgdbmodel.models.{} import {}".format(main_to_text_dict[key][i].capitalize(),
...
def Many_from_text(main_dict):
    ...
    for key in dict_to_type.keys():
        for i in range(len(dict_to_type[key])):
            text = "\nfrom pgdbmodel.models.{} import {}".format(dict_to_type[key][i].capitalize(), dict_to_type[key][i].capitalize())
...
for key in modified_type_dict.keys():
    ...
    for q in range(len(modified_type_dict[key])):
        ...
        if (modified_type_dict[key][q]['PropertyType'] == 'TEXT' or modified_type_dict[key][q][
            ...
            K.append("\n    {}= models.DecimalField(db_column='{}'".format(modified_type_dict[key][q]['db_column'].lower(),modified_type_dict[key][q]['db_column'].lower()))

        elif (modified_type_dict[key][q]['PropertyType'] == 'DATETIME'):
            K.append("\n    {}= models.DateTimeField(db_column='{}'".format(modified_type_dict[key][q]['db_column'].lower(),modified_type_dict[key][q]['db_column'].lower()))
...
# print(final_dict)
final_dict['COMPLETION']=(''.join([str(elem) for elem in n['COMPLETION']]) )
...
#     cl_many = "\nclass {}(models.Model):".format(item_class_list_many[i]['Type'].capitalize())
#     cl_many_1 = "\n    {}=models.ForeignKey({}, on_delete=models.CASCADE)".format(
...
#     cl_f1 = "\nfrom pgdbmodel.models.{} import {}".format(item_class_list_many[i]['ToType'].capitalize(),
#                                          item_class_list_many[i]['ToType'].capitalize())
#     cl_f2 = "\nfrom pgdbmodel.models.{} import {}".format(item_class_list_many[i]['FromType'].capitalize(),
#                                          item_class_list_many[i]['FromType'].capitalize())
#     cn_many = "{}.py".format(item_class_list_many[i]['Type'].capitalize())
#     completeName2 = os.path.join(path, cn_many)
#     f2 = open(completeName2, "w+")
#     f2.write(c + cl_f1 + cl_f2 + cl_many + cl_many_1 + cl_many_2)
#     f2 = ""
#     completeName2 = ""
#     cl_f1 = ""
#     cl_f2 = ""
