...
for key in capitalized_ref_dict.keys():
    for i in range(len(capitalized_ref_dict[key])):
        if (capitalized_ref_dict[key][i]['fk'].lower() == 'network'):
            ref_text = ""
            ref_from_text = ""
        else:
...
domain_list=[]
for key in mapped_domain_dict.keys():
    if('domain_class' in mapped_domain_dict[key][0]):
        domain_list.append(mapped_domain_dict[key][0]["domain_class"])
print(domain_list)
...
to_string_dict = {}
to_string_list = []
for key, z in zip(modified_type_dict.keys(), unique_domain_list):
    ...
    for q in range(len(modified_type_dict[key])):
        if (modified_type_dict[key][q]['PropertyType'] == 'ITEM'):
            ...
            if (modified_type_dict[key][q]['PropertyType'] == 'TEXT' or modified_type_dict[key][q][
                'PropertyType'] == 'CODE'):
                K.append("\n    {}= models.CharField(db_column='{}'".format(modified_type_dict[key][q]['db_column'].lower(),modified_type_dict[key][q]['db_column'].lower()))
...
#print(mapped_domain_dict['TOTALS_DAY'][0]['domain_class'])

# print(domain_list)
#for key in n.keys():
#    final_dict[key] = (''.join([str(elem) for elem in n[key]]))
# print(final_dict)
# final_dict['MTR_READ']=(''.join([str(elem) for elem in n['MTR_READ']]) )
