...
const RenderRow = (props) => {
  ...
  const selected = useSelector(
    (state) => state.workflow?.checkedTags[data[index]?.TAG_ID]
  );
  return (
    <ListItem
      style={style}
...
const Inputs = (props) => {
  ...
  const tags = useSelector((state) => state.workflow?.tags);
  const selectedTags = useSelector((state) => state.workflow?.data?.TAG_ID);
  ...
  React.useEffect(() => {
    setLeft(
      tags.filter((e) => !selectedTags.some((a) => a.TAG_ID === e.TAG_ID))
    );
    setRight(
      tags.filter((e) => selectedTags.some((a) => a.TAG_ID === e.TAG_ID))
    );
  ...
  const handleToggle = (value, val) => {
    dispatch(updateCheckedTags(value.TAG_ID, val));
